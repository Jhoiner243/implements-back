...
export class GananciasServices {
  ...
  private calculateProfitForInvoice(
    factura: FacturasEntity,
    productos: ProductoEntity[]
  ): number {
    return factura.detalles.reduce(
      (sum: number, detalle: (typeof factura.detalles)[0]) => {
  ...
  async getAnaliticasGanancias() {
    ...
    for (const factura of facturas) {
      ...
      for (const detalle of factura.detalles) {
        ...
        const precioCompra: any = productMap.get(detalle.productoId) || 0;
    ...
    const formatData = (map: Map<string, number>) =>
      Array.from(map, ([periodo, ganancias]) => ({ periodo, ganancias })).sort(
