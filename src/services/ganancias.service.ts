...
export class GananciasService {
  ...
  async calculateProfit({tipo_periodo}: {tipo_periodo: TipoPeriodo}): Promise<void> {
    try {
      ...
      for (const factura of facturas) {
        for (const detalle of factura.detalles) {
          ...
          if (precioCompra !== undefined) {
            ...
            gananciaTotal += (detalle.precio - precioCompraSeguro) * detalle.cantidad;
          }
        }
      }

      // Guardar ganancias en la base de datos
      await this.gananciasRepository.createProfit(startDate, endDate, gananciaTotal, tipo_periodo);
