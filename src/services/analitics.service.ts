...
export class AnaliticsService {
  constructor(@inject(AnaliticsRepository) private analiticsRepository: AnaliticsRepository) {}
  async getAnalitIcsPedidos() {
    ...
    return {
        diario: Array.from(dailyCounts, ([periodo, pedidos]) => ({ periodo, pedidos }))
    };
}
  ...
  const weeklyClientes = new Map<string, Map<string, { cliente: string; pedidos: number }>>();
  ...
  pedidos.forEach((pedido) => {
      ...
      const nombreCliente = clientesMap.get(clienteId) || `Cliente ${clienteId}`;
...
async getAnaliticsProductos() {
    ...
    pedidos.forEach((pedido) => {
      ...
      pedido.detalles.forEach((item: { id_producto: string; cantidad: number }) => {
