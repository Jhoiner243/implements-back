...
import { GetNotificationsEntity, NotificationsRepository } from "../repositories/notifications.repository";
...
export class NotificationsService {
  ...
  async activeNotifications(token: string, token_jwt: string): Promise<{message: string}> {
    console.log(token_jwt)
    const idDecoded = jwt.verify(token_jwt, REFRESH_TOKEN_SECRET as string) as { user_id: string };
    console.log("user_id: ", idDecoded.user_id)

      const {message} = await this.notificationsRepository.activeNotifications(token, idDecoded.user_id);


      if (!message) {
        throw new AppError("No se pudo activar la notificación para el usuario", 500);
      }
  ...
  async sendMulticastNotifications(): Promise<void> {
    try {
      ...
      const notificationData: NotificationsEntity = {
        ...
        message:{
        message: `Productos bajos de stock: ${lowStockProducts.map(p => p.nombre).join(", ")}`,
      ...
      if (response.failureCount > 0) {
        notificationData.error = `Errores: ${response.responses.filter(r => !r.success).length}`;
      }

      // 6. Guardar en base de datos usando el repositorio
      await this.notificationsRepository.sendMulticasteNotifications(notificationData);

      console.log(`Notificaciones enviadas: ${response.successCount}`);
      console.log(`Errores: ${response.failureCount}`);

    } catch (error: any) {
      console.error("Error en sendMulticastNotifications:", error)
  ...
  async getForMulicastNotifications(): Promise<GetNotificationsEntity[]> {
    try {
      const notifications = await this.notificationsRepository.getForMulicastNotifications();
      return notifications;
    } catch (error: any) {
      console.error("Error en getForMulicastNotifications:", error);
      throw new Error(`Error al obtener notificaciones: ${error.message}`);
    }
  }

  async notificationDelete(id: string): Promise<{message: string}> {
    const {message} = await this.notificationsRepository.notificationDelete(id);
