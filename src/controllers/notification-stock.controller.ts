...
import { Delete, Get, JsonController, Param, Post, Req, Res } from "routing-controllers";
...
export class NotificationsController implements BaseController{
  ...
  async getMulticastNotifications(@Res() res: Response){
    try {
      const response = await this.notificationsService.getForMulicastNotifications();
  ...
  async ActiveSendNotifications(@Req() req: Request, @Res() res: Response) {  
    try {
      ...
      const {message} =await this.notificationsService.activeNotifications(token, tokenJwt);
  ...
  async notificationDelete (@Res() res: Response, @Param('id') id: string) {
    try {
      const {message} = await this.notificationsService.notificationDelete(id);
      return {message}
    } catch (error){
      if(error instanceof AppError) {
        return res.status(error.status).json({ error: error.message });
